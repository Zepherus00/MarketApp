...
class CatalogAdapter(
    ...
    private var itemsList = emptyList<SaveProductModel>()
    ...
    private fun setupAdapter(holder: CatalogViewHolder, item: SaveProductModel) {
    ...
    private fun setData(holder: CatalogViewHolder, item: SaveProductModel) {
        holder.oldPrice.text = "${item.price.price} ${item.price.unit}"
        ...
        if (item.feedback?.count != null) {
            val rating = item.feedback?.rating.toString()
            val ratingCount = item.feedback?.count.toString()
            holder.rating.text = rating
            holder.countRating.text = ratingCount
        } else {
    ...
    private fun setListeners(holder: CatalogViewHolder, item: SaveProductModel) {
    ...
    fun setProductList(list: List<SaveProductModel>) {
        itemsList = list
        notifyDataSetChanged()
    }
}
